...
function schedule(ms) {
    ...
    schedTimer = setTimeout(function () {
        schedTimer = null;
        if (M) { try { M._iyu_tick(); } catch (e) { console.error('[iyu]', e); } }
    }, Math.max(0, ms));
}
...
function runSource(source) {
    if (!M) {
        var m0 = 'IYU 未初始化, 请先 await IYU.init(...)';
        console.error('[iyu]', m0);
        emit('error', m0);
        return false;
    }
    // 重置界面
    ...
    try {
        ...
        var len = M.lengthBytesUTF8(src);
        var buf = M._malloc(len + 1);
        M.stringToUTF8(src, buf, len + 1);
        var r = M._iyu_run(buf);   // C++ 负责释放
        // 容错模式: 语法问题已在控制台逐条报告, 可解析部分仍会运行; 不拋异常
        return r === 0;
    } catch (e) {
        console.error('[iyu] 运行异常:', e);
        emit('error', '运行异常: ' + (e && e.message || e));
        return false;
    }
}
function resetHost() {
...
var IYU = {
    ...
    init: function (options) {
        ...
        var factoryArg = {
            // wasm 致命异常兜底: 报告到控制台并允许重新 init, 而不是静默死亡
            onAbort: function (what) {
                var msg = 'wasm 运行时异常终止: ' + (what || 'unknown');
                console.error('[iyu]', msg);
                try { emit('error', msg); } catch (e2) {}
            }
        };
        if (o.wasmBinary) {
            // 外部自带 wasm 二进制: 用官方 instantiateWasm 钩子注入 (env 6 的 glue 不读 wasmBinary 参数)
            factoryArg.instantiateWasm = function (info, receiveInstance) {
                WebAssembly.instantiate(o.wasmBinary, info).then(function (out) {
                    receiveInstance(out.instance);
                    return out;
                }).catch(function (e) {
                    console.error('[iyu] wasm 实例化失败:', e);
                    try { emit('error', 'wasm 实例化失败: ' + (e && e.message || e)); } catch (e2) {}
                });
                return {};
            };
        }
        readyP = iyu_runtime(factoryArg).then(function (mod) {
            M = mod;
    ...
    /** 运行裕语言源码 (容错: 语法问题在控制台报告, 可解析部分继续运行; 不拋异常) */
    run: function (source) {
        return readyP.then(function () {
            var r = runSource(source);
            // 至少让一个宏任务过去: 保证启动 tick 完成, 调用方 await 后可安全地连续二次 run
            return new Promise(function (res) { setTimeout(function () { res(r); }, 0); });
        }).catch(function (e) {
            console.error('[iyu] run 异常:', e);
            emit('error', 'run 异常: ' + (e && e.message || e));
            return false;
        });
    },
    /** 从 URL 加载 .iyu 并运行 */
    runUrl: function (url) {
        return readyP.then(function () {
            ...
            console.error('[iyu] runUrl 失败:', e);
            emit('error', 'runUrl 失败: ' + (e && e.message || e));
            return false;
        });
    },
    /** 停止运行 */
