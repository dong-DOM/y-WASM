...
本运行时采用"**报错不熔断**"策略：单条语句的问题不会让整个程序崩溃，错误会逐条上报到浏览器控制台（`[语法]`/`[iyu]` 前缀，带行号）与 `IYU.on('error')` 回调，可解析的部分照常运行。

### 6.1 语法层容错（原版常见写法不再报“莫名错误”）

- **UTF-8 BOM** 自动跳过；**行中 `//` 注释**（手册第 205 行示例形态）。
- **全角符号自动转半角**（仅代码区，字符串内容原样保留）：全角括号 `（）`、逗号 `，`、分号 `；`、冒号 `：`、等号 `＝`、引号 `“”`、加减乘除、全角字母数字、全角/连续空格、NBSP、破折号等——中文输入法误输入不再导致解析失败。
- **未闭合字符串**自动闭合并提示；缺 `)` / 缺 `{` / EOF 缺 `}` 时按行尾隐式闭合或单语句块容错。
- **残缺语句跳过**：一条语句解析失败只跳过该语句，后续代码正常；残缺 `f`/`w` 条件会整体丢弃该条语句（含块体），不会吞掉后续正常代码。
- **宽松点号赋值**：`sss.x = 1` / `ss.x = 1` 与 `sss x = 1` 等价。
- **错误收集上报**：编译期最多收集 200 条（控制台最多显示 50 条），同一条语句的连锁错误去重。

### 6.2 运行层容错（异常参数不再让 wasm 崩溃）

| 防护项 | 上限/行为 |
|---|---|
| 大数转整型（如 `1e30`） | 饱和转换，杜绝 wasm trap（原实现会直接 abort） |
| `nsz(n)` / 数组下标 | 单列表上限 100 万元素，超出截断/忽略并提示 |
| 字符串拼接 / `sr()` 替换膨胀 | 单串上限 32MB，超出截断并提示 |
| `fn` 递归 | 240 层深度上限，超出报错返回 null |
| 表达式嵌套 | 解析 260 层 / 求值 380 层上限 |
| JSON 嵌套 | 解析/序列化 320 层上限（5000 层恶意输入不崩） |
| 线程数量 | 上限 8000，超出拒绝创建并提示；结束的线程自动回收 |
| 对象池 | 上限 400 万，超出复用占位对象 |
| 未知函数 | 同名错误只报前 3 次，避免循环内刷屏 |
| 除零/取模零 | 返回 0；`null + 1` 按原版语义返回 `1`（null 视作 0） |
| 同步上下文遇异步桥 | 报错返回 null，不崩溃 |

### 6.3 API 层容错

- `IYU.run()` **不再抛异常**：编译失败/运行异常返回 `false` 并在控制台与 `onError` 上报；`IYU.runUrl()` 失败同理。
- wasm 实例化失败 / 运行时异常终止通过 `onAbort` 钩子上报，不静默死亡。
- 事件分发（控件点击等）异常被捕获并报告，不再中断页面交互。
- **调度竞争修复**：`iyu_run` 内同步执行主 fiber 首段 + `run()` 返回前等待一个宏任务——连续多次 `IYU.run()` 不再相互吞掉启动代码。

### 6.4 上报通道

| 通道 | 内容 |
|---|---|
| `console.error('[iyu] ...')` | 全部错误（默认开启） |
| `IYU.on('error', fn)` / `onError` 选项 | 错误回调 |
| `IYU.on('log', fn)` / `onLog` 选项 | 日志与错误（带 isErr 标记） |
...
- IYU 运行时 1.1.0 · 裕语言 3.0 语法子集 · 构建：Emscripten `-sMODULARIZE=1 -sEXPORT_ES6=1 -sFILESYSTEM=0 -sALLOW_MEMORY_GROWTH=1 -Oz`
- 1.1.0 变更：全面容错加固（语法容错 / 参数防崩 / 资源上限 / 错误上报 / 调度竞争修复），详见第六章。
