...
在浏览器中直接运行《裕语言 3.0》（iApp）源码的 WebAssembly 运行时。
...
创建手机模拟器框架并加载 WASM 引擎。

| 选项 | 类型 | 默认 | 说明 |
|---|---|---|---|
| `host` | string\|Element | `"body"` | 框架挂载位置（选择器或元素） |
...
- 状态栏/导航条为 DOM 模拟，`uycl` 高级侵入式效果（-5~-10）近似处理
